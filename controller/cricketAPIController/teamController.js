  ...
  try {
    ...
    completeTeamMembersStep: async (req, res) => {
      try {
        ...
        const isAdmin = team.members.some(
          member => member.user.toString() === req.user.userId.toString() && member.role === "admin"
        );
        ...
        if (team.members.length < 2) {
          return res.status(400).json({ error: "Team must have at least one member besides the owner" });
        }
    ...
    assignCaptains: async (req, res) => {
      try {
        ...
        if (!isOwner && !isAdmin) {
          return res.status(403).json({ error: "No permission to assign captains" });
        }
        
        // Check if team is at the right step
        if (team.creationStep < TEAM_CREATION_STEPS.MEMBERS_ADDED) {
          return res.status(400).json({ error: "Please complete adding team members first" });
        }
        ...
        if (!isViceCaptainMember) {
          return res.status(400).json({ error: "Vice-captain must be a team member" });
        }
        
        // Captain and vice-captain cannot be the same person
        if (captainId === viceCaptainId) {
          return res.status(400).json({ error: "Captain and vice-captain cannot be the same person" });
        }
    ...
    completeTeamCreation: async (req, res) => {
      try {
        ...
        if (!isOwner && !isAdmin) {
          return res.status(403).json({ error: "No permission to complete team creation" });
        }
        
        // Check if all required steps are completed
        if (team.creationStep < TEAM_CREATION_STEPS.CAPTAINS_ASSIGNED) {
          return res.status(400).json({ error: "Please assign captain and vice-captain first" });
        }
    ...
    updateTeam: async (req, res) => {
      try {
        ...
        if (!isOwner && !isAdmin) {
          return res.status(403).json({ error: "No permission to update team details" });
        }
...
addTeamMember: async (req, res) => {
  try {
    ...
    if (!isOwner && !isAdmin) {
      return res.status(403).json({ error: "No permission to add team members" });
    }
    
    // Check if team is in the right step to add members
    if (team.creationStep > TEAM_CREATION_STEPS.MEMBERS_ADDED) {
      return res.status(400).json({ error: "Cannot add members after captain assignment" });
    }
    
    // Check if team has reached maximum members limit
    if (team.members.length >= MAX_TEAM_MEMBERS) {
      return res.status(400).json({ error: `Team cannot have more than ${MAX_TEAM_MEMBERS} members` });
    }
    ...
    removeTeamMember: async (req, res) => {
      try {
        ...
        if (team.creationStep > TEAM_CREATION_STEPS.MEMBERS_ADDED) {
          ...
          if (team.captain && team.captain.toString() === req.params.userId || 
              team.viceCaptain && team.viceCaptain.toString() === req.params.userId) {
            return res.status(400).json({ 
              error: "Cannot remove captain or vice-captain after they have been assigned" 
            });
          }
        }
    ...
    getTeamDetails: async (req, res) => {
      try {
        ...
        const isMember = team.members.some(
          member => member.user._id && member.user._id.toString() === req.user.userId.toString()
        );
        const isOwner = team.owner._id && team.owner._id.toString() === req.user.userId.toString();
